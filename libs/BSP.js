...
//               for (let v of s[1].verts) v.world.snap();
    ...
    if (this.isLeaf()) 
    {
        ...
        if (!this._trans_polygons) return;

            for (let p of this._trans_polygons)  
          {
    ...
    renderPortals(engine) {
         ...
         for (let l of this._leaf_list) 
         {
            ...
            for (let p of l._portals) {
                ...
                engine.DrawPolyLined3(p1,new Vector3(0,1,0),20);
                engine.DrawPolyLined2(p2,new Vector3(1,1,0),20);
            }
        }
