class BVHNode {
    constructor() {
        this.bounds = new AABB();
        this.left = null;
        this.right = null;

        this.first = 0;   // Index im Polygon-Array
        this.count = 0;   // Anzahl der Polygone
    }

    isLeaf() {
        return this.count > 0;
    }
}


function buildBVH(polygons, first=0, count=polygons.length) {
    let node = new BVHNode();
    
    
    // Bounds berechnen
    node.bounds.reset();
 
    for (let i = first; i < first + count; i++) {
        let bound=new AABB(polygons[i]);
        node.bounds.Add(bound);
    }


    // Leaf-Kriterium
    if (count <= 4) {
        node.first = first;
        node.count = count;
        return node;
    }

    // Größte Achse
    let size = node.bounds.size();
    let axis = 0;
    if (size.y > size.x) axis = 1;
    if (size.z > size[["x","y"][axis]]) axis = 2;

    
    // Sortieren nach Centroid
    polygons
        .slice(first, first + count)
        .sort((a,b) => a.centroid[["x","y","z"][axis]] - b.centroid[["x","y","z"][axis]]);

    let mid = first + Math.floor(count / 2);

    node.left = buildBVH(polygons, first, mid - first);
    node.right = buildBVH(polygons, mid, count - (mid - first));
    
    return node;
}




function renderPrimitive(primitive, engine, frustum, cam, xform) {

    engine.primitives_in++;

    let res = primitive.plane.Classify(cam.position);
    if (res == BACK) return;

    let result = primitive.ClipByFrustum(frustum);
    if (result[1] < 0 || result[0] === undefined) return;
    if (result[0].verts.length < 3) return;

    let clipped = result[0];
    engine._primitives_cliped++;

    let o = clipped.TransformToScreen(xform);
    if (o[0]) return;

    if (!clipped.Project(engine)) return;

    

    engine._spanrenderer.AddPrimitive(
        clipped,
        engine._dummy_texture
    );
}





const OCCLUDER_MIN_AREA = 80 * 80;

function traverseBVH_OcclusionRender(
    polygons,
    root,
    frustum,
    camera,
    xform,
    viewportW,
    viewportH,
    engine
) {
    const stack = [];
    stack.push(root);

    engine._primitives_in = polygons.length;
    engine._primitives_rendered = 0;
    engine._primitives_mask_culled = 0;
    engine._primitives_aabb_frustum_culled = 0;

    const mask = engine._occlusionMask;
    mask.clear();   // WICHTIG: Maske pro Frame löschen

    while (stack.length > 0) {

        const node = stack.pop();

        // ---- Frustum Culling ----
        if (!frustum.intersectsAABB(node.bounds)) {
            engine._primitives_aabb_frustum_culled++;
            continue;
        }

        // ---- Projection ----
        const bbox = node.bounds.projectAABBToScreen(
            xform, viewportW, viewportH
        );
        

        if(!bbox) continue;

    let minX = Math.floor(bbox.minX);
    let maxX = Math.ceil(bbox.maxX);
    let minY = Math.floor(bbox.minY);
    let maxY = Math.ceil(bbox.maxY);
    let minW = bbox.minDepth; 
    
    const DEPTH_EPSILON = 1e-5;
   
    
    if(engine.isOccludedRectWithPrevZ(minX, maxX, minY, maxY, minW- DEPTH_EPSILON)) {
        engine._primitives_mask_culled++;
       continue;
    }


        // ---- Leaf ----
        if (node.isLeaf()) {
            const end = node.first + node.count;
            for (let i = node.first; i < end; i++) {
            
             if (polygons[i].plane.Classify(camera.position) == BACK) continue;
             let result = polygons[i].ClipByFrustum(frustum);
             if (result[1] < 0 || result[0] === undefined) continue;
             if (result[0].verts.length < 3) continue;
             let clipped = result[0];
             let o = clipped.TransformToScreen(xform);
             if (o[0]) continue;
             if (!clipped.Project(engine)) continue;

 


                // Idealerweise sollte renderPrimitive ein Rect zurückgeben!
                engine._spanrenderer.AddPrimitive(clipped,engine._dummy_texture);
                engine._primitives_rendered++;

              

               
            }

            // ---- Occluder Update ----
         
            continue;
        }

        // ---- Front-to-Back ----
        const left  = node.left;
        const right = node.right;

        if (!left || !right) {
            if (left)  stack.push(left);
            if (right) stack.push(right);
            continue;
        }

        const pL = left.bounds.projectAABBToScreen(xform, viewportW, viewportH);
        const pR = right.bounds.projectAABBToScreen(xform, viewportW, viewportH);

        if (!pL && !pR) continue;
        if (!pL) { stack.push(right); continue; }
        if (!pR) { stack.push(left);  continue; }

        if (pL.minDepth < pR.minDepth) {
            stack.push(right);
            stack.push(left);
        } else {
            stack.push(left);
            stack.push(right);
        }
    }
}

