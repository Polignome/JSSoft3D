...
class BrusFace {
  ...
  SplitByPlane(plane) {
    ...
    for (let i = 0; i < this.verts.length; i++) {
      ...
      var v0 = this.verts[i];
      var v1 = this.verts[j];
      var n0 = this.normals[i];
      var n1 = this.normals[j];
      var res0 = plane.Classify(v0);
      var res1 = plane.Classify(v1);


      if (res0 == COPLANAR) {
        fnpoly.push(new Vector3(n0));
        bnpoly.push(new Vector3(n0));
      ...
      if ((res0 == FRONT && res1 == BACK) || (res1 == FRONT && res0 == BACK)) {
        ...
        var n = new Vector3(n0.x + (scaled * (n1.x - n0.x)),
          n0.y + (scaled * (n1.y - n0.y)),
          n0.z + (scaled * (n1.z - n0.z)));
        ...
        fnpoly.push(new Vector3(n));
        bnpoly.push(new Vector3(n));
      }
    }
    ...
    return [fpoly, bpoly, fnpoly, bnpoly];
  }
  ...
  CreateByPlane(size = 1000000) {
    ...
    this.verts.push(d); this.normals.push(new Vector3(normal));
    this.verts.push(c); this.normals.push(new Vector3(normal));
    this.verts.push(b); this.normals.push(new Vector3(normal));
    this.verts.push(a); this.normals.push(new Vector3(normal));
...
class Brush {
  ...
  Debug() {
    console.log("Debug Brus ")
    console.log("   Faces ", this.face_list.length)
    console.log("   polygons ", this.primitives.length)
    console.log("   Debug Polygons")
    for (let p of this.primitives) {
      console.log("   Polygon verts ", p.verts.length);
  ...
  BuildFaces(keep = 1) {
    ...
    for (let f of this.face_list) {
      ...
      for (let f2 of this.face_list) {
        ...
        if (f2.plane.Classify(f.verts) === SPANNING) {
          ...
          if (help[0]) { f.verts = help[0]; f.normals = help[2]; }
        }
      }
    ...
    for (let f of this.face_list) {
      ...
      for (let i = 0; i < p.verts.length; i++) {
        p.verts[i].normal = new Vector3(f.normals[i]);
      }
