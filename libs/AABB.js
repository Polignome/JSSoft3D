...
class AABB {
    ...
    projectAABBToScreen(xform, viewportWidth, viewportHeight) {
        ...
        //        console.log("       Step1 ", minX, minY, maxX, maxY, anyValid)
    ...
    MakeCube() {
        let ll = this.max.sub(this.min);
        ll.abs();
        let l = Math.max(ll.x, Math.max(ll.y, ll.z)) * 0.5;

        let c = this.center();
        this.reset();

        this.Add(new Vector3(c.x + l, c.y + l, c.z + l));
        this.Add(new Vector3(c.x - l, c.y - l, c.z - l));
    }
    ...
    subdivide(node) {
        ...
        for (let i = 0; i < 8; i++) {
            const min = new Vector3((i & 1) ? cx : this.min.x,
                (i & 2) ? cy : this.min.y,
                (i & 4) ? cz : this.min.z)
            const max = new Vector3((i & 1) ? this.max.x : cx,
                (i & 2) ? this.max.y : cy,
                (i & 4) ? this.max.z : cz)
            subs.push(new AABB(min, max));
        }
        return subs;
    }
    PointInside(v) {
        if ((v.x >= this.min.x && v.y >= this.min.y && v.z >= this.min.z) &&
            (v.x <= this.max.x && v.y <= this.max.y && v.z <= this.max.z)) return true;
        return false;
    }
