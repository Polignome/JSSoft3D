...
class Matrix {
  ...
  normalized() {
    let help = new Matrix(this._N, this._M);
    help.Init(this);
    help.normalize();
    return help;
...
*/
  ...
  static genProjectPerspectiveGluFrustum(fov, aspect, znear, zfar) {
    let ymax = 0;
    let xmax = 0;
    let temp = 0;
    let temp2 = 0;
    ...
    ymax = znear * Math.tan(fov * 3.14159265358979 / 360)
    xmax = ymax * aspect
    ...
    matrix.Data[10] = (-zfar - znear) / temp4
    matrix.Data[11] = -1.0;
    matrix.Data[12] = 0.0;
    matrix.Data[13] = 0.0;
    matrix.Data[14] = (-temp * zfar) / temp4
    matrix.Data[15] = 0.0;
  ...
  static gluUnProject(winX, winY, winZ, modelMatrix, projMatrix, viewport) {
    let finalMatrix = new Matrix4x4();
    let inn = new Array(4);
    let out = new Array(4);

    // Kombiniere Modelview- und Projektionsmatrix
    // OpenGL speichert Matrizen spaltenweise (Column-Major)
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
        finalMatrix.Data[j * 4 + i] =
          modelMatrix.Data[0 * 4 + i] * projMatrix.Data[j * 4 + 0] +
          modelMatrix.Data[1 * 4 + i] * projMatrix.Data[j * 4 + 1] +
          modelMatrix.Data[2 * 4 + i] * projMatrix.Data[j * 4 + 2] +
          modelMatrix.Data[3 * 4 + i] * projMatrix.Data[j * 4 + 3];
      }
    }

    // Invertiere die kombinierte Matrix
    if (!finalMatrix.invert()) return false;

    // Map x und y von window coordinates (0..w, 0..h) zu normalized device coordinates (-1..1)
    inn.Data[0] = (winX - viewport[0]) * 2.0 / viewport[2] - 1.0;
    inn.Data[1] = (winY - viewport[1]) * 2.0 / viewport[3] - 1.0;
    inn.Data[2] = winZ * 2.0 - 1.0;
    inn.Data[3] = 1.0;

    // Multipliziere mit der inversen Matrix
    out.Data[0] = inn.Data[0] * finalMatrix.Data[0] + inn.Data[1] * finalMatrix.Data[4] + inn.Data[2] * finalMatrix.Data[8] + inn.Data[3] * finalMatrix.Data[12];
    out.Data[1] = inn.Data[0] * finalMatrix.Data[1] + inn.Data[1] * finalMatrix.Data[5] + inn.Data[2] * finalMatrix.Data[9] + inn.Data[3] * finalMatrix.Data[13];
    out.Data[2] = inn.Data[0] * finalMatrix.Data[2] + inn.Data[1] * finalMatrix.Data[6] + inn.Data[2] * finalMatrix.Data[10] + inn.Data[3] * finalMatrix.Data[14];
    out.Data[3] = inn.Data[0] * finalMatrix.Data[3] + inn.Data[1] * finalMatrix.Data[7] + inn.Data[2] * finalMatrix.Data[11] + inn.Data[3] * finalMatrix.Data[15];

    if (out.Data[3] == 0.0) return null;

    return new Vector3(out.Data[0] / out.Data[3],
