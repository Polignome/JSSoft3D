class Beam {
    ...
    clipPolygon(poly) {
        ...
        let i = 0;
        for (const plane of this.planes) {
            if (!plane.Classify(poly) === SPANNING) continue;
            let result2 = result.ClipPolyByPlaneNoSnapDebug(plane, result);
            if (!result2) return null;
            //result = result.ClipPolyByPlaneNoSnapDebug(plane, result);
            result = result2[0];
            if (result2[1]) i++;
