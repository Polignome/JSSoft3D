...
function UpdateMouse() {
	...
	WriteToLog("MouseCancasPos ................: "+canvas._mouse_x+" "+canvas._mouse_y+"\n");
	...
	WriteToLog("Objekt at Mousepos.............: "+id+"\n")
/*
    if (GetIndexObjType(id)==2) {
		let index=GetIndexObjIndex(id);
		let p=m_main_portals[index];
	    WriteToLog("Portal              "+index+"\n");
	    WriteToLog("Create in Leaf ID   "+p.leave_id+"\n");
	    WriteToLog("Leaf <              "+p.gestrandet_in+"\n");
	    WriteToLog("Leaf >              "+p.erzeugt_in+"\n");

	}
    if (GetIndexObjType(id)==1) {
		let index=GetIndexObjIndex(id);
	    let p=m_main_primitives[index];
		WriteToLog("Polygon "+index+"\n");
		WriteToLog("Parent Leaf Id "+p._leaf_id+"\n");
		WriteToLog("Parent Node Id "+p._node_id+"\n");
	}
    WriteToLog("\n");
*/
...
function renderLeafs() {
	...
	if (!bsp) return;
     canvas.DrawText(10,10,"Num Leafs "+bsp._leafs.length)
  
	 for (let i=0;i<bsp._leafs.length;i++ ) 
	{
	  let l = bsp._leafs[i];
	  let s = l._aabb.center();
	  let verts=[new Vert(s)];
	  let o=rasterizer.TransformToScreen(verts);
     
	  if (o[0]) continue;
      s=rasterizer.Project(verts);
	  let span=(s[0].x|0)+(s[0].y|0)*rasterizer.width();
      
	 if (rasterizer.GetActiveZBuffer()[span]<s[0].w) {

	  canvas.DrawText(s[0].x,s[0].y,l._leaf_id)
...
//if (bsp) bsp.Render2D(canvas,m_bsp_center.add(new Vector2(canvas.width/2,0))  ,1/m_wheel_delta)
//if (bsp) RenderLeafConections(bsp);
RenderPortals(m_main_portals)
	canvas.Redraw();
//	renderLeafs();
...
function RenderPortals(portals) {
         for (let p of portals) {
			let po=new Polygon(p.verts);
			po.SetColor(1,1,0);
			rasterizer.DrawPolyLined2(po);
...
function RenderLeafConections(bsp) {
  
 



	let leafs=bsp._leafs;
 let color=new Vector3(0,1,0);
 let color2=new Vector3(1,0,0);
	for (let l of leafs) {
        let b=l._aabb.BuildPolygons();
//         for (let p of b) rasterizer.DrawPolyLined(p)

		let c7=l._aabb.center();
         ...
         for (let p of l._portals) {
			...
			rasterizer.DrawPolyLined2(po);
   		    let c8=p.center();
     		rasterizer.DrawLine3D(c7,c8,color);

		let c1=c8.sub(new Vector3(-1,0,0));
		let c2=c8.sub(new Vector3(1,0,0));

   	    let c3=c8.sub(new Vector3(0,-1,0));
		let c4=c8.sub(new Vector3(0,1,0));

   	    let c5=c8.sub(new Vector3(0,0,-1));
		let c6=c8.sub(new Vector3(0,0,1));
        
		rasterizer.DrawLine3D(c1,c2,color2);
        rasterizer.DrawLine3D(c3,c4,color2);
        rasterizer.DrawLine3D(c5,c6,color2);


		 }


		let c1=l._aabb.center().sub(new Vector3(-1,0,0));
		let c2=l._aabb.center().sub(new Vector3(1,0,0));

   	    let c3=l._aabb.center().sub(new Vector3(0,-1,0));
		let c4=l._aabb.center().sub(new Vector3(0,1,0));

   	    let c5=l._aabb.center().sub(new Vector3(0,0,-1));
		let c6=l._aabb.center().sub(new Vector3(0,0,1));
        
		rasterizer.DrawLine3D(c1,c2,color);
        rasterizer.DrawLine3D(c3,c4,color);
        rasterizer.DrawLine3D(c5,c6,color);
		...
		for (let p of l._portals)
		{
			let c2=p._leaf1._aabb.center();
			if (l==p._leaf1) c2=p._leaf2._aabb.center();
            if (l!=p._leaf1 && l!=p._leaf2) DebugOut("ERROR1\n");       
            if (l==p._leaf1 && l==p._leaf2) DebugOut("ERROR2\n");       
			rasterizer.DrawLine3D(c1,c2,color);
		}
...
function ReadMapFromString() {
  ...
  let portals=PortalBuilder.BuildPortals(bsp,true);     // Portale erstellen 
  NonVisPolygonsRemover.Remove(bsp)                // Nicht sichtbare Polygon löschen
  outprims=bsp.ExtractPrims();                    

  bsp=new BSPTree(outprims);      
  portals=PortalBuilder.BuildPortals(bsp,true);     // Portale erstellen 
  outprims=bsp.ExtractPrims();                    



  m_main_primitives=outprims;

  let id=0;
  for (let p of outprims) {
     p._id=CalcIndex(1,0,id);  
     id++;
  }


  m_main_portals=portals;
/*
  for (let p of portals) {
	let pp=new Polygon(p.verts);
	pp._id=CalcIndex(2,0,id);
	id++;
	pp.SetColor(Math.random(),Math.random(),Math.random());
	outprims.push(pp);
  }
*/
