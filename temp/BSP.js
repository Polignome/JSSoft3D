...
function Finde_besten_Splitter(polyliste ,gewichtung = 18) 
{
...
class BSPNode {
  ...
  constructor(primitives,makesolid=false,tree=null,parent=null,owner=0,leaf_id=-1,non_solid=false) {
     ...
     if (non_solid) {this._solid=-1;return;}
     ...
     if (bestsplitter==-1 || makesolid)   
     {
        if (bestsplitter==-1 && !makesolid) 
        {
           ...
           for (let p of primitives) {
            p._leaf_id=this._leaf_id;
            if (p._create_from_aabb)  {this._has_aabb_primitivs =true; }
           }
        ...
        for (let p of primitives) {
            ...
            this._back=new BSPNode(new Array(),false,tree,this,BACK_OWNER,this._leaf_id,true);
            
        }
        
      
        if (found==false) {this._solid=1; }
    ...
    Render2D(canvas,max_scale,depth=0,pos,oldpos=null,scale=0.000001) {
        let iscale=6;
        let m=((max_scale/Math.pow(2,depth+1))*scale)|0;  
        
        

   //       DebugOut(pos.x+" "+pos.y+"\n");
        let color=RGB(128,128,128);

        if (this._solid==1) color=RGB(0,255,0);
        if (this._solid==-1) {color=RGB(255,0,0); return;}
        if (this._node_type==LEAF_NODE) color=RGB(255,255,0);
        
        if (pos.y+2 > 0) 
        canvas.DrawRec(pos.x-2,pos.y-2,pos.x+2,pos.y+2,color);

        if (oldpos) canvas.DrawLine(pos.x,pos.y,oldpos.x,oldpos.y,RGB(255,0,255))


        if (pos.y > canvas.height) return;
        
        
        if (this._front && pos.x<=canvas.width ) this._front.Render2D(canvas,max_scale,depth+1,new Vector2(pos.x+m,pos.y+m),pos,scale) 
        if (this._back && pos.x > 0) this._back.Render2D(canvas,max_scale,depth+1,new Vector2(pos.x-m,pos.y+m),pos,scale) 
    ...
    depth(i=0) {
        let a=0;
        let b=0;
        if (this._front) a=this._front.depth(i+1);
        if (this._back)  b=this._back.depth(i+1);
        
        return Math.max(Math.max(a,b),i);
    }
...
this._prims=[];     
      ...
      ClipPortal(portal,out) {
       ...
       //let portal=this._aabb.ClipPortal(portal);
        ...
        if (result===PLANAR) 
        {
            ...
            if (this._front && this._back) 
            {
                ...
                if (nl>=0) 
                {
                   ...
                   //this._front.ClipPortal(portal,out2,mode); 
                   for (let p of out2) this._back.ClipPortal(p,out);   
                   return   
                }
            }
            if (nl>=0) {
             if (this._front) this._front.ClipPortal(portal,out); //Blos nicht ändern 
            }
...
get total_node_count() {
    ...
    build(primitives) {
        ...
        for (let p of primitives) {
            ...
            p._leaf_id=-1;
            p._id=-1;
            p._leaf_id=p._leaf_id;
            p._node_id=p._node_id;
...
//            let p=this._aabb.ClipPortal(portal);
    ...
    Render2D(canvas,pos2d=new Vector2(0,0),scale=0.000001) {
    
        if (!this._root) return;
        let d=this.depth();
        let s=new Array();
        let m=Math.pow(2,d);  
        this._root.Render2D(canvas,m,0,new Vector2(pos2d.x+canvas.width/2,pos2d.y),null,scale); 
    }
}
